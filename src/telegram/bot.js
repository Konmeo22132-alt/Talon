...
async function executeLoop(chatId, actionType, args, statusMsgId) {
    ...
    for (let step = 0; step < MAX_STEPS; step++) {
        ...
        const silent = ['screenshot', 'grid_screenshot', 'control_mode_on', 'control_mode_off'];
        ...
        await bot.sendChatAction(chatId, 'typing');
        const finalText = await summarizeResult(currentType, result);
    ...
    await bot.sendChatAction(chatId, 'typing');
    const fallbackText = await summarizeResult(lastType, lastResult);
    await editStatus(chatId, statusMsgId, fallbackText);
    addToHistory('assistant', `[Ket qua: ${fallbackText.slice(0, 200)}]`);
}
